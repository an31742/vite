...
const modalMap: any = {
  // 用户表单弹窗（多端适配）
  toLocalStorage: () => import("@/views/demo/toLocalStorage.vue"),
  UserFormModal: () => import("@/views/demo/globalPopup/UserFormModal.vue"),
  DeleteConfirmModal: () =>
    import("@/views/demo/globalPopup/DeleteConfirmModal.vue"),
  // 订单详情弹窗（超大组件：分块打包，减少首屏体积）
  OrderDetailModal: () =>
    import(
      /* webpackChunkName: "modal-order" */ "@/views/demo/globalPopup/OrderDetailModal.vue"
    ),
  // 兜底：未注册的弹窗指向失败组件
  "*": FallbackModal,
};
